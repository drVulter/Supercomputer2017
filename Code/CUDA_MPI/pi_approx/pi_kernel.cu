...
__global__ void pi_kernel(curandState_t* states) {
	const int i = (blockIdx.x * blockDim.x) + threadIdx.x; // Index variable
...
extern "C" long get_circles(int rank, long local_N) {
	...
	int num_iter = local_N / (NUMBLOCKS * TPB);
	printf("num_iter process %d is %d\n", rank, num_iter);
	...
	*/
	...
	for (i = 0; i < num_iter; i++) {
		...
		init<<<NUMBLOCKS, TPB>>>(time(NULL) + rank, states);
		//int *in_circle = (int*)malloc(NUMBLOCKS*TPB * sizeof(int));
		//int *dev_circle;
		//cudaMalloc(&dev_circle, NUMBLOCKS*TPB * sizeof(int));
		cudaMemcpy(counter, host_counter, sizeof(long), cudaMemcpyHostToDevice);
		pi_kernel<<<NUMBLOCKS, TPB>>>(states);
		long iter_count;
